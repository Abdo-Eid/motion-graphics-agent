...
The Planner / Art Director / Implementor agent tasks all assume this layer exists. They do not work without it. The Planner is the supervisor — it dispatches the other agents directly via subagent tools — so the role-guarded helpers here are the only enforcement layer for field ownership.
...
// Audit log only. Dispatch lives in the Planner's tool calls, not in this field.
...
Every helper takes an explicit role argument. A wrong role throws. The delegation-tool layer catches and emits a `field-ownership-violation` event on the bus (see [`phase-3-planner-agent.md`](phase-3-planner-agent.md)).
...
- [`phase-3-planner-agent.md`](phase-3-planner-agent.md) — supervisor + delegation tools that invoke subagents using these helpers; main retrieval consumer
